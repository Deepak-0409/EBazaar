    ...
    async create(req,res) {
        ...
        form.parse(req,async (err, fields, files) => {
            ...
            if(!err)
            {
                ...
                if(parseInt(parsedData.price)<0)
                {
                ...
                for(let i=0; i<Object.keys(files).length; i++)
                {
                    ...
                    {
                        const error ={};
                        error[`msg`]= `image${i+1} has invalid format` 
                        errors.push(error);
                    }
                }
                ...
                if(errors.length === 0)
                {
                    try {
                        ...
                        return res.status(201).json({message: "Product saved succesfully", response});
                    } catch (error) {
                        console.log(error);
    ...
    async updateProduct(req,res){
        ...
        if(errors.isEmpty())
        {
             try {
                ...
                return res.status(200).json({message: 'Product details have been updated succesfully',response});
            } catch (error) {
                console.log(error);
                return res.status(500).json({error:error.message})
            }
